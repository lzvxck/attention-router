...
export function revertedCommit(title: string, body: string | null) {
	return /^Revert ".+"$/.test(title)
		? (body?.match(/This reverts commit ([0-9a-f]{7,40})\./i)?.[1] ?? null)
		: null;
}
...
export function calibration(files: ChangedFile[], stats: RiskStat[]) {
	...
	return [...new Set(files.map((file) => pathPattern(file.filename)))].map(
		(pattern) => {
			const stat = map.get(pattern);
			const rate = stat?.total_prs ? stat.reverted_prs / stat.total_prs : 0;
